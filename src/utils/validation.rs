...
pub fn validate_port(port: u16) -> Result<()> {
    if port == 0 {
        return Err(crate::Error::InvalidPort("Port number must be greater than 0".to_string()));
    }
    Ok(())
}
...
pub fn validate_protocol(protocol: &str) -> Result<()> {
    match protocol.to_lowercase().as_str() {
        ...
        _ => Err(crate::Error::InvalidPort(
            format!("Invalid protocol '{}'. Must be tcp, udp, or all", protocol)
        ))
    }
}
...
pub fn validate_sort_option(sort: &str) -> Result<()> {
    match sort.to_lowercase().as_str() {
        ...
        _ => Err(crate::Error::Other(
            format!("Invalid sort option '{}'. Must be port, pid, or name", sort)
        ))
    }
}
...
mod tests {
    ...
    fn test_validate_sort_option() {
        ...
        assert!(result.is_err());
        assert!(result.unwrap_err().to_string().contains("Invalid sort option"));
    }
}